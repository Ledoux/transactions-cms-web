      ...
      if (!singularOrPluralName) {
        if (!modeName) {
          if (modes && modes.length > 0) {
            dispatch(showModalModesList ? showModalModesList() : (0, _transactionsInterfaceState.showModal)('You need to choose a mode'));
          } else {
        ...
        if (matchingModeName) {
          ...
          if (suggestedMode && modeName !== suggestedMode.name) {
            dispatch((0, _transactionsAuthorizationState.setAuthorizationSelectedMode)(suggestedMode));
          }
        }
      }
    }
