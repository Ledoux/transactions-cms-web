...
var InputForm = function (_Component) {
  ...
  _createClass(InputForm, [{
    ...
    value: function componentDidUpdate() {
      ...
      if (joinValue !== value) {
        dispatch((0, _transactionsReduxReselector.assignReselectorFilter)('WITH_SIGN_JOIN', {
      ...
      if (entity && (isNew || joinId) && entityId !== entity.id) {
        dispatch((0, _transactionsCmsState.mergeForm)(_defineProperty({}, joinCollectionName + 'ById', _defineProperty({}, isNew ? '_NEW_' : joinId, _defineProperty({}, entityName + 'Id', entity.id))), _defineProperty({}, collectionName + 'ById', {
          _NEW_: '_DELETE_'
        })));
      } else if (value === initialValue) {
        // either we refuind the odl initial value
        // so we cancel what we just did
        dispatch((0, _transactionsCmsState.deleteFormEntity)(collectionName, '_NEW_'));
      } else {
        // else we add in the new
        dispatch((0, _transactionsCmsState.mergeFormEntity)(collectionName, entityId || '_NEW_', _defineProperty({}, name, value)));
      }
    }
  }, {
